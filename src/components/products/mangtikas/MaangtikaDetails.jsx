// components/pages/Jewellery/MaangtikaDetails.jsx
...
const MaangtikaDetails = memo(() => {
  ...
  const maangtika = state?.maangtika;
  ...
  const nextImage = useCallback(() => {
    setCurrentImage((prev) => (prev + 1) % maangtika.images.length);
  }, [maangtika.images.length]);

  const prevImage = useCallback(() => {
    setCurrentImage((prev) =>
      prev === 0 ? maangtika.images.length - 1 : prev - 1
    );
  }, [maangtika.images.length]);

  // Pricing calculation
  const goldCost =
    goldPricePerGram && maangtika.netWeight
      ? maangtika.netWeight * goldPricePerGram
      : 0;
    ...
    <div className="min-h-screen px-6 pt-12 pb-8 md:pt-32 md:pb-20 new-font">
      <div className="max-w-5xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden">
        ...
        <div className="p-4 border-b flex items-center justify-between">
          ...
          >
            <ArrowLeft size={20} /> Back to Maangtikas
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 p-6 md:p-10">
          ...
          <div className="relative w-full h-96 rounded-lg overflow-hidden shadow-md bg-gray-100 select-none">
            <AnimatePresence mode="wait">
              <motion.img
                key={maangtika.images[currentImage]}
                src={maangtika.images[currentImage]}
                alt={`${maangtika.name} - view ${currentImage + 1}`}
                draggable={false}
                className={`w-full h-full object-cover transition-all duration-700 ease-in-out ${
                  loadedImages[maangtika.images[currentImage]]
...
export default MaangtikaDetails;
