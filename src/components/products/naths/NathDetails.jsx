// components/pages/Jewellery/NathDetails.jsx
import React, { useState, useCallback, useEffect, memo } from "react";
import { useLocation, useNavigate } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import { ArrowLeft, ChevronLeft, ChevronRight } from "lucide-react";

const NathDetails = memo(() => {
  const { state } = useLocation();
  const navigate = useNavigate();
  const nath = state?.nath; // changed from necklace
  ...
  const nextImage = useCallback(() => {
    setCurrentImage((prev) => (prev + 1) % nath.images.length);
  }, [nath.images.length]);

  const prevImage = useCallback(() => {
    setCurrentImage((prev) => (prev === 0 ? nath.images.length - 1 : prev - 1));
  }, [nath.images.length]);

  // Pricing calculation
  const goldCost =
    goldPricePerGram && nath.netWeight ? nath.netWeight * goldPricePerGram : 0;
    ...
    <div className="min-h-screen px-6 pt-12 pb-8 md:pt-32 md:pb-20 new-font">
      <div className="max-w-5xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden">
        ...
        <div className="p-4 border-b flex items-center justify-between">
          ...
          >
            <ArrowLeft size={20} /> Back to Naths
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 p-6 md:p-10">
          ...
          <div className="relative w-full h-96 rounded-lg overflow-hidden shadow-md bg-gray-100 select-none">
            <AnimatePresence mode="wait">
              <motion.img
                key={nath.images[currentImage]}
                src={nath.images[currentImage]}
                alt={`${nath.name} - view ${currentImage + 1}`}
                draggable={false}
                className={`w-full h-full object-cover transition-all duration-700 ease-in-out ${
                  loadedImages[nath.images[currentImage]]
          ...
          <div>
            ...
            >
              Net Weight: {nath.netWeight ? `${nath.netWeight} grams` : "N/A"}
            </motion.p>
